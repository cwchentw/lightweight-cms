...
sed -i "s/Dual-Mode, Flat-File CMS Powered by PHP/A Concise Description of Your Awesome Site/" %informationConfig:\=/% || (
    echo Unable to modify information.php >&2
    exit /b 1
)

rem Update the site author.
sed -i "s/ByteBard/The Site Author/" %informationConfig:\=/% || (
    echo Unable to modify information.php >&2
    exit /b 1
)
